...
| `uv run lq start @NAME [--adapter TYPE] [--show-thinking]` | 启动（TYPE: `feishu`, `discord`, `telegram`, `local`, 或逗号分隔如 `discord,local`）。`--show-thinking` 为开关（默认关闭），开启后输出工具调用和思考过程 |
...
- **可插拔适配器** — 内置飞书、Discord、Telegram 和本地终端适配器，全部走相同的统一事件管线。可在单一平台运行，也可多平台同时连接
